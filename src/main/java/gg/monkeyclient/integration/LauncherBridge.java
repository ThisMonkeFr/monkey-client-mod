...
public final class LauncherBridge {
 public static final ExecutorService WORK=Executors.newSingleThreadExecutor(r->{var t=new Thread(r,"Monkey launcher UI");t.setDaemon(true);return t;});
 ...
 public static HttpResponse<byte[]> request(String path,JsonObject data)throws Exception{
  ...
  var b=HttpRequest.newBuilder(URI.create(URL+path)).timeout(Duration.ofSeconds(5)).header("Authorization","Bearer "+TOKEN);
